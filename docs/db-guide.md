...
Навыки группы `/db-*` позволяют управлять информационными базами 1С: создавать базы, загружать и выгружать конфигурации, обновлять БД, запускать предприятие, выполнять read-only запросы и загружать изменения из Git.
...
| `/db-query` | `.ps1` | Read-only запрос к данным через COMConnector, JSON/CSV-результат |
...
### Исследовать или независимо проверить данные

```
> Выполни read-only запрос из verify.sql в базе dev, параметры возьми из params.json,
  результат сохрани в artifacts/verify.json
```

Агент вызовет `/db-query`: разрешит базу через `.v8-project.json`, выполнит запрос через
COMConnector и сохранит структурированный результат. Навык не выполняет произвольный BSL-код и
не предоставляет операций записи.
