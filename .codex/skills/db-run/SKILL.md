...
| `-Client Thick/Thin` | нет | Клиент: толстый (`1cv8`, по умолчанию) или тонкий (`1cv8c`) |
| `-Visible` | нет | Оставить окно и стартовые диалоги видимыми для ручной/визуальной проверки |
| `-SingleInstance` | нет | Не запускать второй клиент для той же базы |
| `-WaitReadySeconds <N>` | нет | Ждать готового отвечающего окна до N секунд; код 3 означает таймаут |
| `-ReadyGraceSeconds <N>` | нет | Дополнительное ожидание отвечающего окна, если к таймауту уже появился нормальный заголовок приложения; по умолчанию 30 секунд |
...
Для автоматической визуальной проверки используй `-Client Thin -Visible -SingleInstance -WaitReadySeconds 300`. Для первого запуска тяжёлой конфигурации увеличь ожидание до 600 секунд. Готовность процесса не доказывает, что нужная вкладка открыта и нет модального окна: это следует проверить снимком или UI-сценарием. Обычный фоновый запуск возвращает управление сразу.

Для проверки изменённой формы перед финальной сборкой:

1. Открой её через `-URL` и дождись готовности окна.
2. Убедись, что активна именно нужная вкладка, а не стартовая страница или окно «Информация».
3. Закрой известные стартовые модальные окна тестовой базы (например, интернет-поддержку).
4. Проверь видимость добавленных элементов, основной сценарий и переключатели.
5. При появлении «непредвиденной ситуации» открой подробный текст ошибки; успешный старт процесса такую ошибку не исключает.
