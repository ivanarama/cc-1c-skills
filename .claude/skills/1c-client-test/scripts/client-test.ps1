...
function Get-MainWindowInfo {
    ...
    $window = $windows | Where-Object { $_.Handle -eq $handle.ToInt64() } | Select-Object -First 1
    if (-not $window) { $window = $windows | Sort-Object { $_.Width * $_.Height } -Descending | Select-Object -First 1 }
    return $window
}
