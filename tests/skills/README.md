...
| `check-db-run-readiness.mjs` | Python-порт `db-run`: нормальный заголовок зависшего окна не считается готовностью, а первое splash/modal-окно не скрывает последующее отвечающее окно процесса |
