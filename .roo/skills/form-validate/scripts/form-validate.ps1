...
function Get-DirectChildText {
	...
	$child = $Node.SelectSingleNode("f:$Name", $nsMgr)
	...
	for ($i = 0; $i -lt $Text.Length; $i++) {
		...
		if ($ch -eq '"') {
			...
			if ($inString -and $i + 1 -lt $Text.Length -and $Text[$i + 1] -eq '"') {
				[void]$current.Append($Text[$i + 1]); $i++; continue
			}
			$inString = -not $inString
		} elseif (-not $inString -and $ch -eq '(') {
			$depth++; [void]$current.Append($ch)
		} elseif (-not $inString -and $ch -eq ')') {
			$depth = [Math]::Max(0, $depth - 1); [void]$current.Append($ch)
		} elseif (-not $inString -and $depth -eq 0 -and $ch -eq ',') {
			$item = $current.ToString().Trim()
...
function Normalize-QueryExpr {
	param([string]$Value)
	return ([regex]::Replace("$Value", '\s+', '')).ToLowerInvariant()
}

function Get-AggregateQueryKeyInfo {
	param([string]$Query)
	...
	foreach ($item in (Split-QueryItems $select.Groups[1].Value)) {
		$aliasMatch = [regex]::Match($item, $aliasPattern)
		if ($aliasMatch.Success) {
			$expr = $aliasMatch.Groups[1].Value; $alias = $aliasMatch.Groups[2].Value
		} else {
			$simple = [regex]::Match($item, '(' + $script:dlIdentPattern + ')\s*$')
			if (-not $simple.Success) { continue }
			$expr = $item; $alias = $simple.Groups[1].Value
		}
		$exprAlias[(Normalize-QueryExpr $expr)] = $alias
		if ($expr -match $aggregatePattern) { $aggregateAliases[$alias.ToLowerInvariant()] = $true }
	}

	$groupAliases = @{}
	foreach ($expr in (Split-QueryItems $group.Groups[1].Value)) {
		$key = Normalize-QueryExpr $expr
		if ($exprAlias.ContainsKey($key)) {
			$groupAliases[$exprAlias[$key].ToLowerInvariant()] = $true
		} elseif ($expr.Trim() -match ('^' + $script:dlIdentPattern + '$')) {
			$groupAliases[$expr.Trim().ToLowerInvariant()] = $true
		} else {
			return [pscustomobject]@{ GroupAliases = @(); AggregateAliases = @($aggregateAliases.Keys) }
		}
	}
	return [pscustomobject]@{ GroupAliases = @($groupAliases.Keys); AggregateAliases = @($aggregateAliases.Keys) }
}

function Remove-BslComments {
	param([string]$Text)
	$result = New-Object System.Collections.Generic.List[string]
		...
		for ($i = 0; $i -lt $line.Length; $i++) {
			...
			if ($ch -eq '"') {
				...
				if ($inString -and $i + 1 -lt $line.Length -and $line[$i + 1] -eq '"') {
					[void]$out.Append($line[$i + 1]); $i++; continue
				}
				$inString = -not $inString
			} elseif (-not $inString -and $ch -eq '/' -and $i + 1 -lt $line.Length -and $line[$i + 1] -eq '/') {
				break
			} else {
				[void]$out.Append($ch)
			}
...
function Get-BslRoutines {
	param([string]$Text)
	$clean = Remove-BslComments $Text
	$routines = @{}
	...
	foreach ($match in [regex]::Matches($clean, $pattern)) {
		$name = $match.Groups[1].Value
		$routines[$name.ToLowerInvariant()] = [pscustomobject]@{ Name = $name; Body = $match.Groups[2].Value }
	}
	return $routines
}

function Get-ReachableBslText {
	param($Routines, [string]$Handler)
	$queue = New-Object System.Collections.Generic.Queue[string]
	$queue.Enqueue($Handler.ToLowerInvariant())
	$seen = @{}
	$bodies = New-Object System.Collections.Generic.List[string]
	while ($queue.Count -gt 0) {
		$name = $queue.Dequeue()
		if ($seen.ContainsKey($name) -or -not $Routines.ContainsKey($name)) { continue }
		$seen[$name] = $true
		$body = $Routines[$name].Body
		$bodies.Add($body)
		foreach ($calledKey in $Routines.Keys) {
			$called = $Routines[$calledKey].Name
			if (-not $seen.ContainsKey($calledKey) -and $body -match ('(?i)(?<![A-Za-z0-9_А-Яа-яЁё])' + [regex]::Escape($called) + '\s*\(')) {
				$queue.Enqueue($calledKey)
			}
		}
	}
...
function Get-ServerBslRoutineNames {
	param([string]$Text)
	$clean = Remove-BslComments $Text
	$result = @{}
...
function Test-RoutineReachesServer {
	...
	foreach ($calledKey in $Routines.Keys) {
		...
		if ($body -match ('(?i)(?<![A-Za-z0-9_А-Яа-яЁё])' + [regex]::Escape($calledName) + '\s*\(') -and
			(Test-RoutineReachesServer $Routines $calledKey $ServerNames $Memo $nextVisiting)) {
...
function Get-FirstServerCallPosition {
	param($Routines, [string]$Handler, $ServerNames)
	$key = $Handler.ToLowerInvariant()
	if (-not $Routines.ContainsKey($key)) { return $null }
	$body = $Routines[$key].Body; $memo = @{}; $positions = New-Object System.Collections.Generic.List[int]
	foreach ($calledKey in $Routines.Keys) {
		if (-not (Test-RoutineReachesServer $Routines $calledKey $ServerNames $memo @{})) { continue }
		$calledName = $Routines[$calledKey].Name
...
function Test-DisabledEarlyReturn {
...
function Get-DynamicListToggleHandlers {
	...
	$result = @()
	foreach ($checkbox in $RootNode.SelectNodes('f:ChildItems//f:CheckBoxField', $nsMgr)) {
		$dataPath = $checkbox.SelectSingleNode('f:DataPath', $nsMgr); if (-not $dataPath -or -not $dataPath.InnerText.Trim()) { continue }
		$flagName = $dataPath.InnerText.Trim()
		foreach ($event in $checkbox.SelectNodes('f:Events/f:Event', $nsMgr)) {
			$eventName = $event.GetAttribute('name').ToLowerInvariant()
			if ($eventName -ne 'onchange' -and $eventName -ne 'приизменении') { continue }
			$handler = $event.InnerText.Trim(); $handlerKey = $handler.ToLowerInvariant()
			if (-not $Routines.ContainsKey($handlerKey)) { continue }
...
function Test-NodeInitiallyHidden {
	param($Node, $RootNode)
	$current = $Node
	while ($current -and $current -ne $RootNode) {
		$visible = $current.SelectSingleNode('f:Visible', $nsMgr)
...
function Test-DynamicListParameterSetter {
	param([string]$Text, [string]$AttributeName, [string]$ParameterName)
	$pattern = '(?i)(?<![A-Za-z0-9_А-Яа-яЁё])(?:ЭтаФорма\s*\.\s*|ThisForm\s*\.\s*)?' +
		[regex]::Escape($AttributeName) +
		'\s*\.\s*(?:Параметры|Parameters)\s*\.\s*(?:УстановитьЗначениеПараметра|SetParameterValue)\s*\(\s*"' +
...
# --- Check 15: added arbitrary DynamicList semantics and lifecycle ---
# Параметризованный список, видимый при открытии формы, считывается раньше позднего клиентского
# события. XML и /CheckConfig при этом чистые, а ошибка появляется только в runtime. Lifecycle-
# эвристика применяется только к добавленным реквизитам расширения; проверки формы ключа — ко всем
# корневым произвольным спискам.
if (-not $stopped) {
	$semanticChecked = 0
	$semanticErrorsBefore = $errors
	$semanticWarningsBefore = $warnings

	$baseAttrNames15 = @{}
	if ($baseFormNode) {
		foreach ($baseAttr in $baseFormNode.SelectNodes('f:Attributes/f:Attribute', $nsMgr)) {
			$name = $baseAttr.GetAttribute('name')
			if ($name) { $baseAttrNames15[$name.ToLowerInvariant()] = $true }
		}
	}

	$modulePath15 = Join-Path (Split-Path (Resolve-Path $FormPath).Path -Parent) 'Form\Module.bsl'
	$moduleText15 = if (Test-Path $modulePath15) { [System.IO.File]::ReadAllText($modulePath15, [System.Text.Encoding]::UTF8) } else { '' }
	$routines15 = Get-BslRoutines $moduleText15
	$serverRoutineNames15 = Get-ServerBslRoutineNames $moduleText15
	$allRoutineText15 = (($routines15.Values | ForEach-Object { $_.Body }) -join "`n")
	$checkedToggleHandlers15 = @{}

	$onCreateHandlers15 = @()
	$events15 = $root.SelectSingleNode('f:Events', $nsMgr)
	if ($events15) {
		foreach ($event in $events15.SelectNodes('f:Event', $nsMgr)) {
			$eventName = $event.GetAttribute('name').ToLowerInvariant()
			if (($eventName -eq 'oncreateatserver' -or $eventName -eq 'присозданиинасервере') -and $event.InnerText.Trim()) {
				$onCreateHandlers15 += $event.InnerText.Trim()
			}
		}
	}
	$onCreateBodies15 = @()
	foreach ($handler in $onCreateHandlers15) { $onCreateBodies15 += (Get-ReachableBslText $routines15 $handler) }
	$onCreateText15 = $onCreateBodies15 -join "`n"

	foreach ($attr in $attrNodes) {
		$attrName = $attr.GetAttribute('name')
		$typeValues = @($attr.SelectNodes('f:Type/v8:Type', $nsMgr) | ForEach-Object { $_.InnerText.Trim() })
		if ($typeValues -notcontains 'cfg:DynamicList') { continue }
		$settings = $attr.SelectSingleNode('f:Settings', $nsMgr)
		if (-not $settings -or (Get-DirectChildText $settings 'ManualQuery').ToLowerInvariant() -ne 'true') { continue }
		$semanticChecked++
		$query = Get-DirectChildText $settings 'QueryText'
		$mainTable = Get-DirectChildText $settings 'MainTable'
		$keyType = Get-DirectChildText $settings 'KeyType'
		$keyFields = @($settings.SelectNodes('f:KeyField', $nsMgr) | ForEach-Object { $_.InnerText.Trim() } | Where-Object { $_ })
		$keyFolded = @($keyFields | ForEach-Object { $_.ToLowerInvariant() })

		if (-not $mainTable) {
			if (-not $keyType) {
				Report-Error "15. DynamicList '$attrName': arbitrary query without MainTable needs KeyType"
			} elseif (@('RowKey','FieldValue','RowNumber','Auto') -notcontains $keyType) {
				Report-Error "15. DynamicList '$attrName': unsupported KeyType '$keyType'"
			} elseif (($keyType -eq 'RowKey' -or $keyType -eq 'FieldValue') -and $keyFields.Count -eq 0) {
				Report-Error "15. DynamicList '$attrName': KeyType=$keyType needs at least one KeyField"
			} elseif ($keyType -eq 'FieldValue' -and $keyFields.Count -ne 1) {
				Report-Error "15. DynamicList '$attrName': KeyType=FieldValue needs exactly one KeyField"
			} elseif ($keyType -eq 'RowNumber' -and $keyFields.Count -gt 0) {
				Report-Warn "15. DynamicList '$attrName': KeyField is ignored for KeyType=RowNumber"
			}

			$duplicateKeys = @($keyFolded | Group-Object | Where-Object Count -gt 1 | ForEach-Object Name | Sort-Object)
			if ($duplicateKeys.Count -gt 0) {
				Report-Error "15. DynamicList '$attrName': duplicate KeyField(s): $($duplicateKeys -join ', ')"
			}

			$explicitFields = @{}
			foreach ($value in $settings.SelectNodes('f:Field/*[local-name()="dataPath"]', $nsMgr)) {
				$fieldName = $value.InnerText.Trim()
				if ($fieldName) { $explicitFields[$fieldName.ToLowerInvariant()] = $true }
			}
			if ($explicitFields.Count -gt 0) {
				$unknownKeys = @($keyFields | Where-Object { -not $explicitFields.ContainsKey($_.ToLowerInvariant()) })
				if ($unknownKeys.Count -gt 0) {
					Report-Error "15. DynamicList '$attrName': KeyField(s) absent from declared fields: $($unknownKeys -join ', ')"
				}
			}

			$keyInfo = Get-AggregateQueryKeyInfo $query
			$keySet = @{}; foreach ($key in $keyFolded) { $keySet[$key] = $true }
			if ($keyInfo.GroupAliases.Count -gt 0) {
				$missingGroupKeys = @($keyInfo.GroupAliases | Where-Object { -not $keySet.ContainsKey($_) } | Sort-Object)
				if ($missingGroupKeys.Count -gt 0) {
					Report-Warn "15. DynamicList '$attrName': aggregate query key omits GROUP BY field(s) $($missingGroupKeys -join ', '); row uniqueness is not guaranteed"
				}
			}
			$unstableKeys = @($keyInfo.AggregateAliases | Where-Object { $keySet.ContainsKey($_) } | Sort-Object)
			if ($unstableKeys.Count -gt 0) {
				Report-Warn "15. DynamicList '$attrName': aggregate result field(s) used as KeyField ($($unstableKeys -join ', ')); the row key changes with the aggregate"
			}
		}

		$queryParams = @(Get-QueryParameters $query)
		$isAdded = $hasBaseForm -and -not $baseAttrNames15.ContainsKey($attrName.ToLowerInvariant())
		if (-not $isAdded -or $queryParams.Count -eq 0) { continue }

		$boundTables = @()
		foreach ($table in $root.SelectNodes('f:ChildItems//f:Table', $nsMgr)) {
			if ((Get-DirectChildText $table 'DataPath').ToLowerInvariant() -eq $attrName.ToLowerInvariant()) { $boundTables += $table }
		}
		if ($boundTables.Count -eq 0) { continue }

		foreach ($toggleHandler in @(Get-DynamicListToggleHandlers $root $routines15 $boundTables)) {
			...
			$firstServerCall = Get-FirstServerCallPosition $routines15 $toggleHandler.Handler $serverRoutineNames15
			if ($null -ne $firstServerCall -and -not (Test-DisabledEarlyReturn $toggleHandler.Body $toggleHandler.FlagName $firstServerCall)) {
				Report-Warn "15. DynamicList '$attrName': checkbox handler '$($toggleHandler.Handler)' can reach a local server routine before a recognized disabled-state guard for '$($toggleHandler.FlagName)'; use ``If Not <flag> Then ... Return; EndIf`` before the server-bound call"
			}
			if ($null -ne $firstServerCall) {
				...
				if ($showMatch.Success -and $showMatch.Index -lt $firstServerCall) {
					Report-Warn "15. DynamicList '$attrName': checkbox handler '$($toggleHandler.Handler)' makes the panel visible before the server-bound parameter initialization; initialize first, then show"
				}
				$reachableToggleText = Get-ReachableBslText $routines15 $toggleHandler.Handler
				foreach ($table in $boundTables) {
					...
					if ($reachableToggleText -match $refreshPattern) {
						Report-Warn "15. DynamicList '$attrName': checkbox path explicitly refreshes table '$tableName' after/beside parameter initialization; changing DynamicList.Parameters already schedules reread, so verify and remove the extra refresh"
						break
					}
				}
			}
		...
		foreach ($table in $boundTables) {
			if (-not (Test-NodeInitiallyHidden $table $root)) { $initiallyHidden = $false; break }
		}

		$defaulted = @{}
		foreach ($parameter in $settings.SelectNodes('f:Parameter', $nsMgr)) {
			$nameNode = $parameter.SelectSingleNode('*[local-name()="name"]')
			$valueNode = $parameter.SelectSingleNode('*[local-name()="value"]')
		...
		if ($initiallyHidden) {
			$missingAnywhere = @($queryParams | Where-Object {
				-not $defaulted.ContainsKey($_.ToLowerInvariant()) -and -not (Test-DynamicListParameterSetter $allRoutineText15 $attrName $_)
			})
			if ($missingAnywhere.Count -gt 0) {
				Report-Warn "15. DynamicList '$attrName' is initially hidden, but no parameter setter was found for: $($missingAnywhere -join ', '); initialize before making the bound table visible"
			}
			continue
		}

		$missingOnCreate = @($queryParams | Where-Object {
			-not $defaulted.ContainsKey($_.ToLowerInvariant()) -and -not (Test-DynamicListParameterSetter $onCreateText15 $attrName $_)
		})
		if ($missingOnCreate.Count -gt 0) {
			$tableNames = @($boundTables | ForEach-Object { $_.GetAttribute('name') }) -join ', '
			Report-Warn "15. DynamicList '$attrName' is initially visible in table(s) $tableNames, but OnCreateAtServer does not initialize query parameter(s): $($missingOnCreate -join ', '); initialize them there or keep the table/ancestor Visible=false until initialization"
		}

		$hasOnCreateSetter = $false
		foreach ($parameter in $queryParams) {
			if (Test-DynamicListParameterSetter $onCreateText15 $attrName $parameter) { $hasOnCreateSetter = $true; break }
		}
		if ($onCreateText15 -and $hasOnCreateSetter) {
			foreach ($table in $boundTables) {
				$tableName = $table.GetAttribute('name')
				$refreshPattern = '(?i)(?:Элементы|Items)\s*\.\s*' + [regex]::Escape($tableName) + '\s*\.\s*(?:Обновить|Refresh)\s*\('
				if ($onCreateText15 -match $refreshPattern) {
					Report-Warn "15. DynamicList '$attrName': explicit refresh of table '$tableName' in OnCreateAtServer is redundant before the first display"
					break
				}
			}
		}
	...
	if ($semanticChecked -gt 0 -and $errors -eq $semanticErrorsBefore -and $warnings -eq $semanticWarningsBefore) {
		Report-OK "15. DynamicList semantics/lifecycle: $semanticChecked arbitrary list(s) checked"
	}
}
