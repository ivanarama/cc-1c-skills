...
| `-IbcmdPath <путь>` | нет | Явный путь к `ibcmd`; полезен, если серверные компоненты установлены отдельно |
...
Скрипт ищет `ibcmd` рядом с `1cv8`, затем в стандартной папке той же версии и проверяет совпадение версий. Если утилиты нет, установи компонент «Сервер 1С:Предприятия» из дистрибутива той же версии либо передай `-IbcmdPath`. Копировать одиночный exe другой версии нельзя.

Пакетные операции конфигуратора автоматически повторяются до трёх раз при кратковременной блокировке базы завершающимся процессом.
