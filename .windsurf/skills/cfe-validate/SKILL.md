...
Для произвольных динамических списков валидатор проверяет ключ результата без `MainTable` и предупреждает о виртуальной таблице регистра в роли `MainTable`, которую платформа может отвергнуть в добавленном реквизите формы расширения.

Также валидатор отвергает попавшие в `QueryText` BSL-префиксы строк `|` и предупреждает о подозрительном обращении `ДинамическийСписок.ТекущаяСтрока`. Последнее является эвристикой: для текущей строки таблицы формы обычно используй `Элементы.<ИмяТаблицы>.ТекущиеДанные` и обязательно проверяй обработчик в клиенте — статическая проверка не доказывает корректность runtime API.

Для добавленных параметризованных `DynamicList` проверяется порядок: все параметры до первого показа, без лишнего `Обновить()` при создании формы. Если флажок управляет скрытой панелью, его клиентский `OnChange` должен сделать `Если Не <флаг> Тогда ... Возврат; КонецЕсли` до локального вызова, достигающего `&НаСервере`. Анализ не следует в общие модули и остаётся эвристикой; оба состояния флажка всё равно проверяй в клиенте.
