...
| `-AllowRunningClients` | нет | Явно продолжить, если эту файловую базу уже используют процессы 1С |
| `-StopRunningClients` | нет | Перед загрузкой запросить штатное закрытие обычных клиентов этой файловой базы |
| `-ClientStopTimeoutSeconds <N>` | нет | Ждать штатного закрытия клиентов до N секунд (1–300, по умолчанию 30) |

> `*` — нужен либо `-InfoBasePath`, либо пара `-InfoBaseServer` + `-InfoBaseRef`

### Preflight файловой базы

Перед загрузкой скрипт проверяет уже запущенные процессы 1С. Совпадение считается найденным
только когда значение аргумента `/F` или `--db-path` после нормализации равно полному пути
`-InfoBasePath`; похожий путь вроде `Dev-Copy` не блокирует `Dev`. По умолчанию при точном
совпадении скрипт печатает PID и режим процесса и завершается до изменения базы.

- `-StopRunningClients` отправляет `WM_CLOSE` только процессам в режиме `ENTERPRISE` и ждёт их
  завершения. Принудительное завершение не выполняется. Designer, `ibcmd` и процесс с
  неопределённым режимом нужно закрыть отдельно.
- `-AllowRunningClients` — явное решение продолжить без закрытия. Не совмещай его с
  `-StopRunningClients`.
- Проверка действует для файловой базы. Она видит процессы, в командной строке которых есть путь
  базы; соединение через зарегистрированное имя (`/IBName`) или сторонний COM-процесс этим способом
  достоверно сопоставить с каталогом нельзя.
...
При итеративной доработке большой конфигурации не повторяй полную загрузку основной конфигурации. Для расширения загружай только его каталог через `-Extension`; если платформа и состав изменения допускают частичную загрузку, используй `-Mode Partial`. Сначала добейся успешного открытия изменённой формы в клиенте, затем выполняй финальную полную проверку применимости, синтаксиса и выгрузку CFE.
