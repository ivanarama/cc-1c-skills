...
```powershell
powershell.exe -NoProfile -File ".codeassistant/skills/form-edit/scripts/form-edit.ps1" -FormPath "<путь>" -JsonPath "<путь>"
```
...
Если запрос содержит параметры `&Имя`, держи связанную таблицу или её группу с `"visible": false` до установки всех параметров. `form-edit` меняет только `Form.xml` и не генерирует BSL, поэтому видимая параметризованная таблица даёт предупреждение до записи XML.
